...
- URP 2D project, git.
- Defs as plain C# classes: `BuildingDef, ResidentDef, FragmentDef, RecipeDef`. A `ContentDb` singleton holds lists; loaded by a bootstrap scene.
- Runtime/save as plain classes: `GameState { day, cash, heat, rep, buildings{}, residents{}, batches[], inventory{}, flags }` + `SaveVersion`.
- Install Newtonsoft JSON package (`com.unity.nuget.newtonsoft-json`) — JsonUtility can't do dictionaries. Save to `Application.persistentDataPath`.
- Static `GameEvents`: `BuildingStateChanged, FragmentFound, ResidentMovedIn, DayEnded, HeatChanged, RepChanged`.
- Rules: defs immutable, managers talk only via events + GameState.
- Done: boots, defs load, log prints 6 buildings.

## Phase 1 — Street + building states (wk 1–2)
- Player: Rigidbody2D (kinematic) + box collider, 8-dir, ~90 px/s @ 16px tiles. Street walk end-to-end ≈ 12–15 s.
- Tilemap: ground / collision / overhead (sorting layers). Placeholder tiles only.
- `Building.prefab` (×6, data-driven by BuildingDef ref): · door trigger collider · Light2D per window (on at Restored) · dev label (name/state/price).
- Interaction: one key, nearest trigger wins. Route by state: buy → clear (stub: instant) → advance stage.
- `BuildingManager` mutates runtime, fires `BuildingStateChanged`; prefab listens and swaps its own sprite/lights.
- Add purchase juice now: sound + scale tween (cheap, teaches the pipeline).
- Done: walk, buy all 6 with debug cash, states cycle grey→green, window appears. Play 10 min.

## Phase 2 — Production + money (wk 3–4)
- **Still minigame in its own scene** (iterate there, not in-world):
...
- Ferment: batches tick with game hours. Vat interactable: empty → choose recipe (consume ingredients) → fermenting → ready → minigame. Second vat purchasable mid-slice.
- Selling: Tormod knocks (list price) · traveling cart 2 of 3 days (sells ingredients, buys bottles) · risky buyer event (2× price, +15 heat, 10% confiscation at heat > 50).
- HUD: cash, day, clock, heat, rep.
- Done: cart → mash → ferment → still → sell, no debug keys; 15 min of loop is near-fun.

## Phase 3 — Renovation + day cycle + save (wk 5–6)
- Day: 1 real s ≈ 1.3 game min (~15 min days). Global Light2D color lerps noon→dusk→night. Window lights now pay off — screenshot the first lit window.
- Sleep → tick, strict order: ferment → Elias repairs → income → heat decay → move-in checks → autosave. Comment the order; never reorder casually.
