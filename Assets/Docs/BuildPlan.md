...
- Systems: movement/interaction · building states · staged construction · production (mash → ferment → bottle) · **roadside stand + request book (the primary economy)** · homestead site projects (Phase U) · day-night + sleep-save · night beats · conspiracy trust · recruitment beats · public infrastructure · JSON save (built in Phase 9 — see Rules).
...
**Phase S is partly done, not done.** Built 2026-07-26: the request domain (`Rules/StandRequest`, `RequestBook`, `RequestBookRules`, `RequestArrivalRules`), `StandManager` bridging clock and inventory, the `Stand` interactable and the IMGUI `RequestBookUI`. All three are placed in SampleScene — `StandManager` at root, `RequestBookUI` on `HUDCanvas` beside the other panels, `Stand` on the camp clearing at `(-17.25, 8)`. Still open, each its own later plan: replies and the correspondence voice (notes carry placeholder signatures only), the strangers → residents customer mix, descriptive requests pointing past what the player can brew, ~~slot upgrades beyond the starting 3~~ (**moved to Phase U** — Stand II grants 3→5 via the existing `RequestBook.SetSlotCount`), the town storefront, retiring Tormod as a channel, and appointments as demand events. `RequestArrivalRules.DescriptiveInN = 4` is an **invented** tuning number — GameDesign.md says only "a minority are descriptive" and does not fix the fraction.

## Phase U — Homestead site projects
Design source: GameDesign.md Part 3, "The homestead site" (`docs/superpowers/specs/2026-08-05-homestead-upgrades-design.md`). Foraged-material stages depend only on Phase 1 tech; Nails/Barrels/Still-Parts stages depend on Phase F (Smithy hand-production). **Materials only — the material list is the gate; no locks, no level requirements.**
- [ ] Project spots placed on the homestead plot, each its own staged build reusing the shell's stage tech (`Homestead.cs` pattern). A project can be started (foraged stages) before it can be finished (factory-good stages) — the half-built shed is the signpost pointing at the Smithy.
- [ ] Storeroom: Crates (Wood) → Shelving (Wood + Nails). Home stash capacity. Crates are the cheapest post-shell project — buildable the same day the shell closes.
- [ ] Porch & fire: Fire ring (Stone) → Bench (Wood) → Lamp (Nails). No mechanics; this is the furniture night beats stage on (coordinate with Phase N).
- [ ] Brewing shed: Footing (Stone) → Frame (Wood) → Fit-out (Nails + Still Parts) → **vat #2** online.
- [ ] Stand II: Shelves (Wood) → Awning & board (Wood + Nails) → request slots 3→5 (`RequestBook.SetSlotCount`) and arrivals 2→3/night, the settled 2026-07-26 numbers.
- [ ] The cellar: Dig & line (Stone) → Shore & stairs (Wood + Nails) → Racks (Wood + Barrels) → **barrel aging** unlocked; aged requests start arriving. Must land before the Mill reveal (Phase 7 ordering).
- [ ] Vat #3: Shed extension (Wood + Still Parts). Late — after the cellar, once aged batches park in the pipeline.
- [ ] Furnished rooms: 2-3 interior stages (Wood, Nails, town goods). Pure expression, zero mechanics.
- [ ] **Triage invariant holds at every tree state:** the average day leaves at least one order unfilled. Vat #2 + Stand II land together by construction (both nail-gated); the demand knob is request size drifting toward 2-3 batches.
- [ ] **Open tuning, deliberately unsettled:** stage material quantities · aged ferment duration (proposal 2-3 game days) · rack capacity (2-4 barrels) · request-size drift curve.
- [ ] Done: the moment the shell closes, the player can see 3-4 startable projects at different price points, and the cadence walkthrough in the spec holds in the build.
...
- [ ] Recruitment beats on move-in coroutine tech: Tormod (Act 0), Berta, Signe, Aksel, Ingrid, Elias. **Berta, Aksel, and Ingrid's move-in destinations retarget from their operated building to their player-built house (2026-08-04 — tech survives, destination changes; see Phase H).** Runa, the miller, is a new recruitment beat, arriving via the Boarding House.
...
- [ ] Runa, the miller: arrives via the Boarding House, builds a house, works the player's Mill. The endgame hire.
